...
storage_num = int(pallet[" X"].shape[0])
...
storage_id = np.full(num_nodes, np.inf)  # 结点标签为索引，值为储位节点标签
agv_label = []  # agv结点标签
pick_label = []  # 拣选结点标签
for i in range(num_nodes):
    if map['#TYPE'][i] == 5:
    ...
    if map["#TYPE"][i] == 2:
        ...
        for j in range(storage_num):
            if map[" X"][i] == pallet[" X"][j] and map[" Y"][i] == pallet[" Y"][j]:
                storage_id[i] = j
...
pick_load = np.zeros(pick_num)  # 拣选结点的负载量
...
order_aver = order_sum / pick_num  # 订单总量的平均值
...
def find_pick(pre_loc):
    ...
    for i in range(pick_num):
        pick_dist.append(distance_matrix.iloc[pre_loc, pick_label[i]])
    return min(pick_dist)
...
class Individual:
    ...
    # q2:结合拣选工位负载要素分配拣选工位
    def select_pick(self, pre_loc):
        best_pick_label = 0
        fit_pick_dist = np.full(pick_num, np.inf)  # 到负载满足要求的拣选结点距离
        min_dist = distance_matrix.iloc[pre_loc, pick_label[0]]
        for i in range(pick_num):
            if self.pick_load[i] < order_aver:  # 当前拣选结点负载量小于平均负载量
                pre_dist = distance_matrix.iloc[pre_loc, pick_label[i]]
                fit_pick_dist[i] = pre_dist
                min_dist = min(fit_pick_dist)
                best_pick_label = np.argmin(fit_pick_dist)
        x = map[" X"][pre_loc]
        y = map[" Y"][pre_loc]
        self.pick_load[best_pick_label] += pallet_sum[int(storage_id[pre_loc])]
        return min_dist, x, y, best_pick_label

    # 计算个体适应度
    def evaluate_fitness(self):
        self.pick_load = np.zeros(pick_num)
        x_list = []
        y_list = []
        pick_list = []
        agv_loc = np.zeros(19)
        ...
        for frag in gene_list:
            ...
            dist = self.select_pick(int(dest))
            x_list.append(dist[1])
            y_list.append(dist[2])
            pick_list.append(pick_label[dist[3]])
            if agv_loc[pre] == 0:
                ...
                fitness += 2 * dist[0]
                agv_loc[pre] = dest
        for pick_node in pick_label:
            x_list.append(map[" X"][pick_node])
            y_list.append(map[" Y"][pick_node])
            pick_list.append(pick_node)
        plt.scatter(x_list, y_list, c=pick_list, cmap='rainbow', marker='s')
